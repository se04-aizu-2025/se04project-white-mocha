...
async function onPickCsvFile(file: File) {
  ...
  const [highlight, setHighlight] = useState<{ i: number; j: number; type: "COMPARE" | "SWAP" } | null>(null);
...
// null: checking, true: online, false: offline
  ...
  <span style={{ fontSize: 14 }}>
    API: {apiOnline === null ? "checking..." : apiOnline ? "online" : "offline"}
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        <label>
          Algorithm:
          <select value={algoKey} onChange={(e) => setAlgoKey(e.target.value)} style={{ marginLeft: 8 }}>
            {algos.length === 0 ? (
              <>
        ...
        <label>
          Array (CSV):
          <input value={input} onChange={(e) => setInput(e.target.value)} style={{ marginLeft: 8, width: 240 }} />
        </label>
  ...
  />
      ...
      {error && (
        <div style={{ marginTop: 10, padding: 10, border: "1px solid #a33", borderRadius: 8, color: "#ffb3b3" }}>
          {error}
        </div>
      )}
      ...
      {run && (
        <div style={{ marginTop: 10 }}>
          ...
          <div>
            <b>Highlight:</b> {highlight ? `${highlight.type} (${highlight.i}, ${highlight.j})` : "-"}
          </div>
        </div>
      )}

      <div style={{ marginTop: 14, border: "1px solid #444", borderRadius: 8, padding: 8 }}>
        <svg width={width} height={height} style={{ display: "block" }}>
          {array.map((v, idx) => {
            ...
            const fill = isHi ? (highlight!.type === "SWAP" ? "#ffb020" : "#4aa3ff") : "#888";

            return (
              <g key={idx}>
                <rect x={x + 2} y={y} width={Math.max(0, barW - 4)} height={h} rx={4} fill={fill} />
                <text
  x={x + barW / 2}
